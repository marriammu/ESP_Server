...
const char *server_url = "http://172.28.130.66/Sensors";// Nodejs application endpoint
...
void loop() 
{
  ...
  // Reading from the sensor and printing the vals
  ...
  // Transmitting the data to the node.js server through wifi and http 
  
  //Create a json dataype named Values to keep the readings
  JsonObject& values = jsonBuffer.createObject();
  //Hold the values
  values["Temperature"] = temp;
  values["Humidity"] = humidity; // tb3 el server
